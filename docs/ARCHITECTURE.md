...
└─ admin/
    ...
    ├─ index              لوحة التحكم بأربعة تبويبات:
    │                     نظرة عامة · الأنشطة · المحتوى · المشاركون
    ├─ activity/[id]      إدارة نشاط: تعديل، حالة التسجيل، رمز الحضور، حذف
...
| 21 | لوحة إدارة (Dashboard + CRUD) | ✅ منفّذ |
...
| 6 | لوحة تحكم الإدارة (4 تبويبات + إدارة نشاط + مسابقة + توعية + إعلان + إشعار) | ✅ منفّذة |
...
ضغطة مطوّلة (ثانية تقريبًا) على اسم التطبيق أسفل شاشة «حسابي» تفتح بوابة الإدارة،
ثم يُطلب رمز الإدارة. لا يوجد أي زر ظاهر يدل عليها للمستخدم العادي.
...
عرض نسبة الإشغال والمقاعد المتبقية، توليد/حفظ رمز الحضور، معاينة الشاشة كما يراها
المستخدم، وحذف النشاط بتأكيد.
