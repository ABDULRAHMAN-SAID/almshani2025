...
# ── لوحة الأعداد
# ── صفّ الإيقاف والسرعة (§7)
top=panelrect(ROOT,(0,1),(24,-24),(300,56))
pb=panelrect(top,(1,0.5),(-8,0),(108,44),(62,69,81,235))
label(pb,(0.5,0.5),(0,0),(100,34),'إيقاف',22,ink,'c')
for i,sp in enumerate(['١×','٢×','٣×']):
    b=panelrect(top,(0,0.5),(8+i*58,0),(54,44),(76,58,38,240) if i==0 else (62,69,81,235))
    label(b,(0.5,0.5),(0,0),(48,34),sp,22,ink,'c',False)

cnt=panelrect(ROOT,(0,1),(24,-96),(300,108))
...
# ── أزرار القدرات (§8)
for i,(nm,hk,x,fillv) in enumerate([('رشقة الفجر','Q',24,1.0),('راية الحشد','E',154,0.45),('الضوء الأوّل','R',284,0.72)]):
    a=panelrect(ROOT,(0,0),(x,24),(122,122),(14,16,19,235) if fillv>=1 else (62,69,81,235))
    fr2=rect(a,(0.5,0.5),(0,0),(110,110)); fb2=box(fr2)
    fb2[1]=fb2[3]-110*fillv
    d.rectangle(fb2,fill=(224,191,115) if fillv>=1 else (123,96,48))
    label(a,(0.5,0.5),(0,-6),(116,60),nm,22,ink,'c')
    label(a,(0,1),(8,-4),(34,26),hk,18,ink,'c',False)
...
label(ROOT,(1,0),(-24,600),(420,40),'٣ فرقةً تتبعك',26,gold,'r')
label(ring,(0.5,0.5),(-166,372),(420,32),'اضغط الأوامر مطوّلاً لاختيار النوع',20,ink,'c')
fr=rect(ROOT,(1,0),(-24,470),(396,56))
for nm,ox,on in [('الجميع',-8,True),('حرّاس',-140,False),('رماة',-272,False)]:
    b=panelrect(fr,(1,0),(ox,0),(124,56),(76,58,38,240) if on else (14,16,19,235))
    label(b,(0.5,0.5),(0,0),(116,36),nm,24,ink,'c')
