...
(async () => {
  ...
  const tap=async sel=>{ await page.waitForSelector(sel,{timeout:180000,state:'attached'}); await page.evaluate(q=>{ const e=document.querySelector(q); if(e) e.click(); }, sel); await page.waitForTimeout(300); };
  await tap('#ovBtn'); await tap('#loStart'); await page.waitForTimeout(900);
