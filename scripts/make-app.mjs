...
// فحص سريع لمنطق التطبيق — ثوانٍ، ويشمل الوضعين. أخطاء التاريخ وتوحيد
// الأرقام لا تظهر في الاستعمال العادي، وتظهر عند أول جهاز منطقته مختلفة.
say();
const units = spawnSync(process.execPath, ["scripts/test-units.mjs"], { stdio: "inherit" });
if (units.status !== 0) {
  fail("منطق التطبيق فيه خلل — لم نبنِ شيئًا", [
    "الأسطر أعلاه تسمّي القاعدة التي اختلّت وما حدث فعلًا.",
    "أصلحها ثم أعد: npm run make-app",
  ]);
}
...
if (wantsDemo) {
  ...
  // فحص محلي فوري: هل كل استعلام في التطبيق يطابق المخطط؟ خطأ في اسم عمود
  // يعمل في الوضع التجريبي ويفشل على الخادم، ولا يكلّف كشفه هنا شيئًا.
  const queries = spawnSync(process.execPath, ["scripts/check-queries.mjs"], { stdio: "inherit" });
  if (queries.status !== 0) {
    say();
    fail("استعلامات التطبيق لا تطابق المخطط — لم نبنِ شيئًا", [
      "الأسطر أعلاه تسمّي الجدول والعمود في كل حالة.",
      "أصلحها ثم أعد: npm run make-app",
    ]);
  }

  say();
  say(DIM("  نتأكد أن الجداول والدوال والحاويات موجودة فعلًا على خادمك..."));
  say();
